...
/**
 * Created by story on 2017/3/13 0013 上午 10:25.
...
public class AlertView {
    ...
    private void initViews() {
        ...
        rootView = (ViewGroup) layoutInflater.inflate(R.layout.alert_view_ios_main, decorView, false);
        ...
        switch (style){
            case ActionSheet:
                ...
                margin_alert_left_right = context.getResources().getDimensionPixelSize(R.dimen.alert_view_ios_sheet_left_right);
            ...
            case Alert:
                ...
                margin_alert_left_right = context.getResources().getDimensionPixelSize(R.dimen.alert_view_ios_alert_left_right);
    ...
    private void initHeaderView(ViewGroup viewGroup){
        //标题和消息
        TextView tvAlertTitle = (TextView) viewGroup.findViewById(R.id.tv_title);
        TextView tvAlertMsg = (TextView) viewGroup.findViewById(R.id.tv_msg);
        if(title != null && !title.equals("")) {
            tvAlertTitle.setText(title);
            if (style == Style.Alert && (msg == null || msg.equals(""))) {
                tvAlertTitle.setTextSize(TypedValue.COMPLEX_UNIT_PX, context.getResources().getDimension(R.dimen.alert_view_ios_alert_txt_header_size_when_msg_null));
            }
        }else{
    ...
    private void initListView(){
        ListView alertButtonListView = (ListView) contentContainer.findViewById(R.id.list_buttons);
        //把cancel作为footerView
        if(cancel != null && style == Style.Alert){
            View itemView = LayoutInflater.from(context).inflate(R.layout.alert_view_ios_button, null);
            TextView tvAlert = (TextView) itemView.findViewById(R.id.tv_button);
            tvAlert.setText(cancel);
            tvAlert.setClickable(true);
            tvAlert.setTypeface(Typeface.DEFAULT_BOLD);
            tvAlert.setTextColor(ContextCompat.getColor(context, R.color.alert_view_ios_txt_cancel));
            tvAlert.setBackgroundResource(R.drawable.alert_view_ios_bg_button_bottom);
    ...
    private void initActionSheetViews(LayoutInflater layoutInflater) {
        ViewGroup viewGroup = (ViewGroup) layoutInflater.inflate(R.layout.alert_view_ios_sheet,contentContainer);
        initHeaderView(viewGroup);
        initListView();
        TextView tvAlertCancel = (TextView) contentContainer.findViewById(R.id.tv_cancel);
    ...
    private void initAlertViews(LayoutInflater layoutInflater) {
        ViewGroup viewGroup = (ViewGroup) layoutInflater.inflate(R.layout.alert_view_ios_alert, contentContainer);
        initHeaderView(viewGroup);
        int position = 0;
        //如果总数据小于等于HORIZONTAL_BUTTONS_MAXCOUNT，则是横向button
        if(mDatas.size()<=HORIZONTAL_BUTTONS_MAXCOUNT){
            ViewStub viewStub = (ViewStub) contentContainer.findViewById(R.id.view_horizontal);
            viewStub.inflate();
            LinearLayout loAlertButtons = (LinearLayout) contentContainer.findViewById(R.id.lay_buttons);
            for (int i = 0; i < mDatas.size(); i ++) {
                //如果不是第一个按钮
                if (i != 0){
                    //添加上按钮之间的分割线
                    View divider = new View(context);
                    divider.setBackgroundColor(ContextCompat.getColor(context, R.color.alert_view_ios_divider));
                    LinearLayout.LayoutParams params = new LinearLayout.LayoutParams((int)context.getResources().getDimension(R.dimen.alert_view_ios_divider), LinearLayout.LayoutParams.MATCH_PARENT);
                ...
                if(mDatas.size() == 1) {
                    ...
                    tvAlert.setBackgroundResource(R.drawable.alert_view_ios_bg_button_left);
                } else if(i == mDatas.size() - 1) {//设置最右边的按钮效果
                    tvAlert.setBackgroundResource(R.drawable.alert_view_ios_bg_button_right);
                }
            ...
            ViewStub viewStub = (ViewStub) contentContainer.findViewById(R.id.view_vertical);
            viewStub.inflate();
            initListView();
        }
    }
